...
final class AnalyticsService: AnalyticsServiceProtocol {
    ...
    func calculateSleepAnalytics(events: [SleepEvent], for baby: Baby, dateRange: SleepAnalytics.DateRange) -> SleepAnalytics {
        let completedSleeps = events.filter { !$0.isOngoing }
    ...
    func calculateFeedingAnalytics(events: [FeedEvent], for baby: Baby, dateRange: SleepAnalytics.DateRange) -> FeedingAnalytics {
        ...
        let totalVolume = events.compactMap { $0.volume }.reduce(0, +)
        let averageDaily = totalVolume / Double(days)
        let averagePerFeed = events.isEmpty ? 0 : totalVolume / Double(events.count)
        ...
        if events.count > 1 {
            ...
            averageInterval = totalInterval / Double(events.count - 1)
        }

        return FeedingAnalytics(
            babyId: baby.id ?? "",
            dateRange: dateRange,
            totalFeedings: events.count,
            averageFeedingsPerDay: Double(events.count) / Double(days),
